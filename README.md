...
`POST /api/mes/insights/bottleneck` 比较标准周期与实际周期，并结合排队工单、停机时长和资源利用率生成瓶颈分数。接口区分正常、关注和严重等级，输出节拍校准、工序分流、维护工单或产能扩充建议，适合用于车间看板的异常排序。
...
新增 `POST /api/mes/ai/defect-root-cause`，把缺陷率漂移、温度、振动、工装寿命、人员切换和物料批次切换关联成风险分、候选根因和围堵动作。无模型时按可解释规则运行；部署方配置 DeepSeek 或 OpenAI 兼容大模型后，可按“人、机、料、法、环、测”生成验证假设与排查步骤。

检索关键词：AI MES、智能制造系统、AI 质量分析、缺陷根因分析、制造异常诊断、DeepSeek MES、工业 AI、知华科技 MES。
